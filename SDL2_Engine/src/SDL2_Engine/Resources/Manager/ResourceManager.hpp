...
namespace SDL2_Engine {
	namespace Resources {
		...
		template<class T>
		using SDL2Resource = std::shared_ptr<__SDL2Resource<T>>;
		...
		**/
		class Resources {
			...
			std::unordered_map<resourceID, std::shared_ptr<ResourceBase>> mResourceLookup;
			...
			void loadResource(std::shared_ptr<ResourceBase> pResource);
		...
		*/
		...
		static SDL2Resource<T> Resources::load(const char* pFilepath, TArgs ... pArgs) {
			...
			static_assert(std::is_base_of<ResourceBase, __SDL2Resource<T>>::value, "Can not load a Resource that is not of the SDL2_Resource type");
			...
			if (iter == mInstance->mResourceLookup.end()) {
				...
				SDL2Resource<T> temp = SDL2Resource<T>(new __SDL2Resource<T>(ID, pArgs...));

				//Add the new resource to the map
				mInstance->mResourceLookup.insert(std::pair<resourceID, std::shared_ptr<ResourceBase>>(ID, temp));
			...
			else return std::dynamic_pointer_cast<__SDL2Resource<T>>(iter->second);
		}
