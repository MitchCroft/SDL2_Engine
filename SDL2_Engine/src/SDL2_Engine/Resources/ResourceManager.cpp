...
//! Included for current time information
#include <chrono>
...
//! Include the Endian information
...
namespace SDL2_Engine {
	namespace Resources {
		...
		*/
		void ResourceManager::recurseResources(std::string pFilepath, std::vector<std::string>& pToFill) const  {
		...
		*/
		void ResourceManager::loadResource(std::shared_ptr<ResourceTypes::ResourceBase> pResource) {
			...
			case EResourceType::Texture: {
				...
				//Check if the Texture is being created from a Surface
				if (res->mToConvert) {
					//Convert the surface
					temp = SDL_CreateTextureFromSurface((SDL_Renderer*)mRenderReference, res->mToConvert);

					//Unload the SDL_Surface
					res->freeMemory();

					//Check the texture was loaded properly
					if (!temp) {
						res->mStatus = EResourceLoadStatus::Error;
						mErrorCB("Failed to convert the custom SDL_Surface to an SDL2Resource<Texture> object (ID: %i). ERROR: %s", res->mID, SDL_GetError());
						return;
					}
				}

				//Otherwise attempt to load the resource
				else {
					//Load the image
					temp = IMG_LoadTexture((SDL_Renderer*)mRenderReference, TO_LOAD.c_str());
			...
			case EResourceType::Cursor: {
				...
				if (Utilities::Endian::SYSTEM_END_ORDER) {
					rmask = 0x000000ff;
					gmask = 0x0000ff00;
					bmask = 0x00ff0000;
					amask = 0xff000000;
		...
		*/
		bool ResourceManager::init(const char* pDirectory, const SDL_Renderer* pRenderer, DebugCallback pWarningCB /* = nullptr */, DebugCallback pErrorCB /* = nullptr */) {
			...
			if (!(mInstance = new ResourceManager())) {
		...
		*/
		std::string ResourceManager::idToFilepath(const resourceID& pID) {
		...
		/*
			Resources : generateTexture - Generate an SDL2Resource<Texture> object from a given SDL_Surface 
			Author: Mitchell Croft
			Created: 07/03/2017
			Modified: 07/03/2017

			NOTE:
			DO NOT DELETE the surface after passing it to this function. The Surface will be
			freed after the resource loading process

			param[in] pSurface - The SDL_Surface object to convert to a Texture
			param[in] pBlendMode - The blend mode to apply to the 

			return SDL2Resource<Texture> - Returns a Texture contained in a 
		*/
		SDL2Resource<Texture> ResourceManager::generateTexture(SDL_Surface* pSurface, const int& pBlendMode) {
			//Generate the resourceID value
			const resourceID ID = mInstance->generateID<Texture>(pSurface, pBlendMode, std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count());

			//Create the new resource
			SDL2Resource<Texture> temp = SDL2Resource<Texture>(new ResourceTypes::__SDL2Resource<Texture>(ID, pBlendMode));

			//Lock the resource items
			std::lock_guard<std::recursive_mutex> guard(mInstance->mLock);

			//Look for a matching resource (Should not be found)
			auto iter = mInstance->mResourceLookup.find(ID);

			//If a resource is found
			if (iter != mInstance->mResourceLookup.end()) {
				//Set error status
				temp->mStatus = EResourceLoadStatus::Error;

				//Output the error message
				mInstance->mErrorCB("Failed to convert the SDL_Surface at location %p to an SDL2Resource<Texture> object as a matching ID was found. ID: %i", pSurface, ID);
			}

			//Otherwise add the setup values
			else {
				//Add the surface pointer to the SDL2Resource
				temp->mToConvert = pSurface;
				...
				mInstance->mResourceOrigin.insert(std::pair<resourceID, std::string>(ID, "SDL2Resource<Texture> generated from an SDL_Surface object"));
			}
