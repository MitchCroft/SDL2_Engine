	...
	namespace Resources {
		...
		**/
		...
		public:
			...
			//! Converter
			static SDL2Resource<Texture> generateTexture(SDL_Surface* pSurface, const int& pBlendMode);
		};
		...
		*/
		template<class T, typename ... TArgs>
		static SDL2Resource<T> ResourceManager::load(const char* pFilepath, TArgs ... pArgs) {
			...
			if (iter == mInstance->mResourceLookup.end()) {
				...
				temp->mErrorCB = mInstance->mErrorCB;
		...
		*/
		template<typename T, typename ... TArgs>
		resourceID ResourceManager::generateID(TArgs ... pArgs) {
			std::ostringstream o;
		...
		*/
		template<typename T, typename ... TArgs>
		void ResourceManager::build_string(std::ostream& pStream, const T& pFirst, const TArgs& ... pArgs) {
			pStream << pFirst;
