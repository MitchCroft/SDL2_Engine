...
namespace SDL2_Engine {
	...
	*/
	bool StateManager::init(const StateManagerInitValues& pSettings) {
		//Create a copy of the settings descriptor
		StateManagerInitValues descript = pSettings;

		//Check the debug outputs are valid
		if (!descript.warningCallback) descript.warningCallback = simpleOutput;
		if (!descript.errorCallback) descript.errorCallback = descript.warningCallback;
		...
		if (!(mInstance = new StateManager())) {
			//Output error
			descript.errorCallback("Failed to create the SDL2_Engine::StateManager singleton");
		...
		mInstance->mCmdArgs = descript.commandArgs;
		mInstance->mCmdCount = descript.commandArgsCount;

		//Assign the function pointers
		mInstance->mInitFunc = descript.initFunc;
		mInstance->mMainFunc = descript.mainFunc;
		mInstance->mThreadFunc = descript.threadFunc;
		mInstance->mDestroyFunc = descript.destroyFunc;

		//Assign the debug callback functions
		mInstance->mWarningCB = descript.warningCallback;
		mInstance->mErrorCB = descript.errorCallback;
	...
	void StateManager::run() {
		...
		if (mInstance->mThreadFunc) {
			...
			mInstance->mThread = std::thread([&]() {
				...
				mInstance->mThreadTime = new Time::Time();
		...
		if (mInstance->mMainFunc) {
			...
			mInstance->mMainTime = new Time::Time();
